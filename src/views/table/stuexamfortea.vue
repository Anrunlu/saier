    ...
    <el-row :gutter="6">
      ...
      <el-col :span="18">
        <div>
          ...
          <el-button type="danger" icon="el-icon-edit" @click="extendExamTime">延长任务时间</el-button>
          <br>
          <font color="red" size="12">当前页面分数：{{ itemScore }}</font><br>
          ...
          <el-checkbox-group v-model="checkedQues" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="item in queses" :key="item.value" :label="item.label">{{ item.label }}</el-checkbox>
          </el-checkbox-group>

          <el-button type="warning" style="width:40%" @click="hwsubmit">
            提交批改
          </el-button>
    ...
    <div>
      ...
      <div v-for="(item,index) in quesAnswers" :key="index" class="editor-container" @click="showidx(index)">
        <div style="border:3px solid #00F">
          <p align="center"><font color="red">第{{ item.father }}大题  题目：{{ index+1 }} (满分：{{ item.sliderMax }})</font></p>
          <mavon-editor :ref="item.itemname" v-model="item.question" :editable="hwswitch" height="300px" @imgAdd="$imgAdd" />

          <el-row :gutter="8">
            <el-col :span="8">
              <el-input v-model="item.stuscore" type="value" disabled size="small" />
            </el-col>
            <el-col :span="16">
              <el-input v-model="item.stucomment" autosize size="medium" type="textarea" placeholder="请输入内容" />
            </el-col>

          </el-row>
          <el-row>
            <el-col>
              <el-slider v-model="item.stuscore" :max="item.sliderMax" @change="scorechange" />
            </el-col>
...
import { recImg, getstuhwfortea, pigaistuhw, extendExamTime, pigaiStuOwnExam, hwOpenSwitch, hwCloseSwitch, changedaan, getStuOwnExamForTea } from '@/api/hwexp'
import 'mavon-editor/dist/css/index.css'
 
//var userno=JSON.parse(decodeURIComponent(window.atob(localStorage.getItem("userno")))); 
...
export default {
  ...
  mounted() { 
    ...
    // }
    // this.cur_page = localStorage.getItem('useridx')

    // console.log(localStorage.getItem('examidx'))
    this.getData()
  },
  methods: {
    extendExamTime() {
      const data = { 'owner': localStorage.getItem('userno'), 'userno': this.userlist[this.cur_page], 'coures': this.selcoures, 'examidx': localStorage.getItem('examidx'), 'stuclass': '', 'exttime': this.examYmdTime }
    ...
    handleCheckAllChange(val) {
      this.checkedQues = []
      if (val) {
        for (let i = 0; i < this.queses.length; i++) {
          this.checkedQues.push(this.queses[i].label)
        }
      }
      // this.checkedQues = val ? this.queses : []
      this.isIndeterminate = false
      this.getData()
    },
    handleCheckedCitiesChange(value) {
      const checkedCount = value.length
      this.checkAll = checkedCount === this.queses.length
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.queses.length
      this.getData()
    },
    changeQues() {
    ...
    scorechange() {
      ...
      for (let i = 0; i < this.quesAnswers.length; i++) {
        this.itemScore = this.itemScore + this.quesAnswers[i].stuscore
      }
      // this.itemScore=this.itemScore+this.quesAnswers[this.mavonidx].stuscore
      // this.ratecomm = ''
      // this.scorecomm = this.score2comment(this.quesAnswers[this.mavonidx].stuscore)
      // // console.log(this.mavonidx)
      // this.quesAnswers[this.mavonidx].comment = this.scorecomm + this.ratecomm
    },
    rate2comment(rate) {
    ...
    getData() {
      const data = { 'userno': this.userlist[this.cur_page], 'coures': localStorage.getItem('stucoures'), 'examidx': parseInt(localStorage.getItem('examidx')), 'owner': localStorage.getItem('userno'), 'quesidx': this.checkedQues }
     
      getStuOwnExamForTea(data).then(response => {
		 
        this.quesAnswers = []
        this.quesInfo = []
        this.itemScore = 0
        for (let i = 0; i < response.data.examinfo.length; i++) {
          this.quesAnswers = this.quesAnswers.concat(response.data.examinfo[i].quesmany)
          for (let j = 0; j < response.data.examinfo[i].quesmany.length; j++) {
            this.itemScore = this.itemScore + response.data.examinfo[i].quesmany[j].stuscore
          }
          // console.log(response.data.examinfo[i].quesmany.stuscore)
          this.quesInfo.push(response.data.examinfo[i].quesmany.length)
          this.queses.push({ 'value': i, 'label': i + 1 })
        }

        // console.log(this.quesInfo)
        // this.quesAnswers = response.data.examinfo[0].quesmany
        ...
        this.queses = []
        for (let i = 0; i < response.data.examnum; i++) {
          // this.quesAnswers = this.quesAnswers.concat(response.data.examinfo[i].quesmany)
          this.queses.push({ 'value': i, 'label': i + 1 })
        }
    ...
    hwsubmit() {
      // console.log(this.quesAnswers)
      this.getTime()
      const data = { 'userno': this.userlist[this.cur_page], 'coures': localStorage.getItem('stucoures'), 'examidx': parseInt(localStorage.getItem('examidx')), 'answer': this.quesAnswers, 'owner': localStorage.getItem('userno'), 'quesInfo': this.checkedQues }
      // console.log(data)
      pigaiStuOwnExam(data).then(response => {
        // this.tableData = response.data
        this.$message(response.data)
        setTimeout(() => {
          this.listLoading = false
